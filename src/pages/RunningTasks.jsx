...
function RunningTasks() {
  ...
  const tasks = data?.tasks?.filter(task => task?.status === 'Running' && useDateFormatter(new Date(task?.createdAt)) === useDateFormatter(new Date(date)));

  if (isLoading) return <TaskSkeleton />

  return (
    <div className='p-3'>
      <Title title={"Taskboom - Running Task"} />
      {/* New tasks header */}
      ...
      {/* New tasks data section */}
      {
        tasks?.length <= 0 ? (
          <Default
            ...
            desc={"All your daily running tasks will be appear here and also can filter by date."}
          />
        ) : (
